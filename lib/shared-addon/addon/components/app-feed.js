...
import layout from '../templates/components/app-feed';
...
export default class AppFeed extends Component {
  ...
  didReceiveAttrs() {
    this.filterOptions.push(...this.extraFilters);
    this.cache.get('app-feed-filter').then(filter => {
      this.set('selectedFilter', filter || FILTERS.ALL);
      if (this.interestType) {
        this.cache.get(`${this.interestType}-notice`).then(value => {
    ...
    if (newActivities.length > 0) {
      run(() => {
        newActivities.forEach(activity => {
          ...
          // Don't consider it a new activity if the author of the action is the session user
          if (type === 'Post' || type === 'Comment' && (activity.actor.split(':')[1] === this.session.currentUser.remoteId)) {
            if (type === 'Post') {
              // If we already have a `activityGroup` in the records list that doesn't have a `remoteId`
              // that means this realtime event came after the record being created in the store.
